...
export default function AdminDashboard() {
  ...
  if (loading) return <Typography sx={{ textAlign: 'center', py: 6, color: '#8a7f76' }}>Загрузка…</Typography>

  const statCards = [
    { label: 'Всего клиентов', value: stats.totalUsers, icon: UsersIcon },
    { label: 'Новых сегодня', value: stats.newToday, icon: UsersIcon },
    { label: 'Новых за месяц', value: stats.newMonth, icon: UsersIcon },
    { label: 'Просмотров страниц', value: stats.totalVisits, icon: ChartIcon },
    { label: 'Кликов в WhatsApp', value: stats.whatsappClicks, icon: WhatsAppIcon },
    { label: 'Кликов в Instagram', value: stats.instagramClicks, icon: InstagramIcon },
    { label: 'Отправлено заявок', value: stats.formSubmits, icon: LeadsIcon },
    { label: 'Сейчас на сайте', value: stats.onlineNow, icon: UsersIcon },
  ]
  ...
  return (
    <div>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>Активность за 7 дней</Typography>
              ...
              <Box key={i} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'flex-end', height: 160 }}>
                  <motion.div initial={{ height: 0 }} animate={{ height: Math.max(4, d.visits * 10) }} transition={{ delay: i * 0.1, duration: 0.5 }}
                    style={{ width: 20, background: '#d4a857', borderRadius: '4px 4px 0 0' }} title={`Просмотры: ${d.visits}`} />
                  <motion.div initial={{ height: 0 }} animate={{ height: Math.max(4, d.clicks * 10) }} transition={{ delay: i * 0.1 + 0.1, duration: 0.5 }}
                    style={{ width: 20, background: '#e8915a', borderRadius: '4px 4px 0 0' }} title={`Клики в WhatsApp: ${d.clicks}`} />
                </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 1.5, fontSize: '0.8rem' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><Box sx={{ width: 12, height: 12, background: '#d4a857', borderRadius: 0.75 }} /> Просмотры</Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><Box sx={{ width: 12, height: 12, background: '#e8915a', borderRadius: 0.75 }} /> Клики в WhatsApp</Box>
          </Box>
        </CardContent>
      </Card>
      ...
      <Grid container spacing={2.5}>
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2 }}>Последние регистрации</Typography>
              {recentUsers.length === 0 ? <Typography sx={{ textAlign: 'center', py: 4, color: '#8a7f76' }}>Клиентов пока нет</Typography> : (
                <TableContainer component={Paper} sx={{ background: 'transparent' }}>
                  <Table size="small">
                    <TableHead><TableRow><TableCell sx={{ color: '#8a7f76', fontWeight: 600 }}>Имя</TableCell><TableCell sx={{ color: '#8a7f76', fontWeight: 600 }}>Email</TableCell><TableCell sx={{ color: '#8a7f76', fontWeight: 600 }}>Дата</TableCell></TableRow></TableHead>
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2 }}>Последние заявки</Typography>
              {recentLeads.length === 0 ? <Typography sx={{ textAlign: 'center', py: 4, color: '#8a7f76' }}>Заявок пока нет</Typography> : (
                <TableContainer component={Paper} sx={{ background: 'transparent' }}>
                  <Table size="small">
                    <TableHead><TableRow><TableCell sx={{ color: '#8a7f76', fontWeight: 600 }}>Имя</TableCell><TableCell sx={{ color: '#8a7f76', fontWeight: 600 }}>Телефон</TableCell><TableCell sx={{ color: '#8a7f76', fontWeight: 600 }}>Статус</TableCell></TableRow></TableHead>

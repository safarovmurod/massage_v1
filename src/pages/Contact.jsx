...
import { TextField, MenuItem, Button as MuiButton, Box, Typography, Alert, CircularProgress } from '@mui/material'
...
export default function Contact() {
  ...
  const handleWA = (e) => {
    ...
    window.open(getWhatsAppLink('ru'), '_blank', 'noopener,noreferrer')
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (name.length < 2) { setError(t('contact.form.err.name')); return }
    if (phone.replace(/\D/g, '').length < 9) { setError(t('contact.form.err.phone')); return }

    // Бот заполнил скрытое поле — тихо показываем успех, в базу ничего не пишем
    ...
    try {
      const { error: dbError } = await supabase.from('leads').insert({
        ...
        message: form.comment.trim().slice(0, 500) || null,
        preferred_time: form.time.trim().slice(0, 80) || null,
      ...
      setError(t('contact.form.err.send'))
    } finally {
      setLoading(false)
    }
  }
            ...
            <div>
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
                ...
                <div className="flex items-start gap-3.5 mb-4">
                  ...
                  <div><div className="text-xs text-muted-soft uppercase tracking-wide">WhatsApp</div><div className="text-base font-semibold"><a href={getWhatsAppLink('ru')} target="_blank" rel="noopener noreferrer" onClick={handleWA}>+992 007 336 264</a></div></div>
                </div>
                <div className="flex items-start gap-3.5 mb-5">
                  ...
                  <div><div className="text-xs text-muted-soft uppercase tracking-wide">Instagram</div><div className="text-base font-semibold"><a href="https://www.instagram.com/orom.massage.tj/" target="_blank" rel="noopener noreferrer" onClick={() => trackInstagramClick()}>@orom.massage.tj</a></div></div>
                </div>
                <motion.a href={getWhatsAppLink('ru')} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="btn btn-primary btn-block mb-2.5" onClick={handleWA}><WhatsAppIcon size={18} /> Написать в WhatsApp</motion.a>
                <a href="https://www.instagram.com/orom.massage.tj/" target="_blank" rel="noopener noreferrer" className="btn btn-secondary btn-block" onClick={() => trackInstagramClick()}><InstagramIcon size={18} /> Открыть Instagram</a>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
                ...
                <a href={routeLink} target="_blank" rel="noopener noreferrer" className="btn btn-outline btn-block">Построить маршрут</a>
              </motion.div>
                  ...
                  {!submitted ? (
                    <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      ...
                      <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                        ...
                        <TextField
                          label="Website" value={website} onChange={e => setWebsite(e.target.value)}
                        ...
                        <MuiButton
                          ...
                          sx={{
                            ...
                            '&:hover': { background: 'linear-gradient(135deg, #e8c178, #e8915a)' },
                            '&.Mui-disabled': { background: 'linear-gradient(135deg, rgba(212,168,87,0.5), rgba(232,145,90,0.5))', color: 'rgba(255,255,255,0.7)' },
                          }}
                        >
                          {loading ? <CircularProgress size={22} sx={{ color: '#fff' }} /> : 'Отправить заявку'}
                        </MuiButton>
                      </Box>
                    </motion.div>
                  ) : (

...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(''); setSuccess('')
    setLoading(true)
    try { await resetPassword(email); setSuccess(t('auth.reset.sent')) }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-5" style={{ backgroundImage: 'linear-gradient(180deg, #1a1520 0%, #221c2a 50%, #1a1520 100%)' }}>
      ...
      >
        ...
        <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="text-3xl font-bold text-center mb-2 gradient-text">{t('auth.reset')}</motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-sm text-muted-soft text-center mb-7">{t('auth.forgot')}</motion.p>
            ...
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                ...
                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                  ...
                  >
                    {loading ? <CircularProgress size={22} sx={{ color: '#fff' }} /> : t('auth.reset.btn')}
                  </MuiButton>
                </motion.div>
              </Box>
            </motion.div>
